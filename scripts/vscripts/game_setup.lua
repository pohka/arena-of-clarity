  ...
  GameRules:GetGameModeEntity():SetItemAddedToInventoryFilter(
		function(ctx, event)
			...
			if unit ~= nil then
				...
				if sub == "item_boots_tier_" then
					local num = string.sub(itemName, -1)
					local tier = tonumber(num)

					--get current tier by name
					local currentTier = 0
					if unit:HasItemInInventory("item_boots_tier_1") then
						currentTier = 1
					elseif unit:HasItemInInventory("item_boots_tier_2") then
						currentTier = 2
					elseif unit:HasItemInInventory("item_boots_tier_3") then
						currentTier = 3
					end
					...
					if currentTier < tier then
						...
						if currentTier > 0 then
							local curItemName = "item_boots_tier_" .. currentTier
